      ...
      <div class="row">
        <div class="col s3 no-padding">
          <div class="leftChoose" v-setheight:style="wh">
            <router-link :to="{ path: '/buySch', query: {ID:'HOT'}}">
            <div class="Hot" >热门</div></router-link>
            <router-link  v-for="item in characters" :key="item.text"  :to="{ path: '/buySch', query: {ID:item.text}}"><div>{{item.text}}</div></router-link>
          </div>
        </div>
...
export default {
  ...
  created: function (){
      if(!this.$route.query.ID){
        this.$router.replace({path: '/buySch', query:{ID: 'HOT'}});
      }else{
